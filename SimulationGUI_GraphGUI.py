...
class SimulationGUI_GraphGUI():
	...
	def __init__(self, _tools):
		...
		self.graph_frame.place(x=0, y=self.canvas.cget("height"), height=self.graph_GUI_height, width=self.canvas.cget("width"))
	...
	def BuildGraph(self):
		...
		for each_value_type in [eval_total] + list(eval_varialbes):
			...
			self.graph_animations.append(mp_ani.FuncAnimation(each_graph, self.GraphAnimation, fargs=(each_value_type,), interval=1000))